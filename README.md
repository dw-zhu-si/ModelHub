...
正常外部调用不会为了刷新目录而重复拉取或测试模型。只有用户在应用中主动执行单模型、单供应商或“测试全部”时，才会重新读取目录并按协议探测；测试聊天模型可能产生供应商费用，媒体和专用动作模型不会被错误送到聊天端点。
...
- 支持手工价格来源与更新时间、月度 Token 配额、预算预警和硬限制；
- 价格未知时保持“未知”，不会伪造费用；
...
1. 启动 ModelHub，在“模型供应商”中添加供应商、Base URL、API Key 和模型名称。
